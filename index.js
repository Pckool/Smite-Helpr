...
client.on("message", async (message) => {
	...
	if (args[0] === "ping") {
		...
		if (message.member.voice.channel) {
			...
			if (args[1] === "balanced") {
				...
				let msg = await message.channel.send(`**${output}**`);

				addReactiveReactions(msg, '🔀', rerollGod);
				...
				var output = `Spread Team (${randomN(0)} Hunter, ${randomN(1)} Guardian, ${randomN(2)} Mage, ${randomN(3)} Warrior, ${randomN(4)} Assassin)\n`;
		...
		if (playerIdRegx.test(args[1])) {
			try {
				...
				let msg = await message.reply(`You requested to trade **${trader.god}** for **${tradee.god}** from <@!${tradee.player.id}>.\n**To accept, use \`s!accept\`**`);
				addReactiveReactions(msg, '👎', () => {})
				addReactiveReactions(msg, '👍', () => {})
	...
	} else if (args[0] === "decline" || args[0] === "deny") {
		let tradeRequest = tradeRequests.find(tradeRequest => tradeRequest.tradee.player.id === message.member.id);
		
		if (tradeRequest) {
			_.pullAt(tradeRequests, tradeRequests.indexOf(tradeRequest));
			let output = 'Trade denied!';
		...
		try {
			let new_god = getRandom(getGods(), args[1]);
			currentTeam.find(teamMember => teamMember.player.id === message.member.id).god = new_god;

			let output = 'You Rerolled!\n__New Team__\n';
			for (let i = 0; i < currentTeam.length; i++) {
				output += `${currentTeam[i].player} ${currentTeam[i].god}\n`;
			}
			message.reply(`**${output}**`);
...
function getGods(category=undefined){
	try {
		...
		console.warn("Retrieval of gods file failed...");
		console.error(err);
	}
}
/**
...
function addReactiveReactions(msg, emoji, fnAdd, fnRm=undefined){
	msg.react(emoji).then(console.log).catch(console.error);
	const filter = (reaction, user) => reaction.emoji.name === emoji;
	let reactions = msg.createReactionCollector(filter)
...
async function rerollGod(r, user){
	let team_member = currentTeam.find(teamMember => teamMember.player.user.id === user.id);
	
	if (!team_member) return; // a dirty error check
	team_member.god = getRandom(getGods());
	r.message.reply(`${formatId(user.id)} rerolled! \n${printTeam()}`);
	let reaction = await r.remove();
	reaction.message.react(reaction.emoji);
}

function printTeam(teamId=undefined){
	let output = `__Team${teamId?' '+teamId:''}__\n`;
	for (let teamMember of currentTeam){
		console.log(`${formatId(teamMember.player.id)} ${teamMember.god}\n`);
		output.concat(`${formatId(teamMember.player.id)} ${teamMember.god}\n`);
	}
	return `**${output}**`
}
...
async function run(){
	...
	setInterval(async () => {
		await updateGods();
	}, 7*24*60*60*1000) // updates every week
